...
namespace Stockfish {
	...
	bool Bitbases::probe(const Square wksq, const Square wpsq, const Square bksq, const Color us) {
	...
	namespace {
		...
		KPKPosition::KPKPosition(const unsigned idx) : stm(static_cast<Color>((idx >> 12) & 0x01)),
			psq(make_square(static_cast<File>((idx >> 13) & 0x3), static_cast<Rank>(RANK_7 - ((idx >> 15) & 0x7))))
		{
