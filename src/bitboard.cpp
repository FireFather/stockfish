...
namespace Stockfish {
	...
	void Bitboards::init() {
		...
		for (Square s1 = SQ_A1; s1 <= SQ_H8; ++s1)
			for (Square s2 = SQ_A1; s2 <= SQ_H8; ++s2)
				SquareDistance[s1][s2] = static_cast<uint8_t>(std::max(distance<File>(s1, s2), distance<Rank>(s1, s2)));
	...
	namespace {
		...
		void init_magics(const PieceType pt, Bitboard table[], Magic magics[]) {
			...
			const int seeds[][RANK_NB] = { { 8977, 44560, 54343, 38998,  5731, 95205, 104912, 17020 },
