...
#define sync_endl std::endl << IO_UNLOCK
	...
	static inline constexpr union { uint32_t i; char c[4]; } Le = { 0x01020304 };
	...
	public:
		...
		void set(const int64_t p, const int64_t q)
		{
			average = p * PERIOD * RESOLUTION / q;
		}

		// Update average with value v
		void update(const int64_t v)
		{
			average = RESOLUTION * v + (PERIOD - 1) * average / PERIOD;
		}

		// Test if average is strictly greater than rational a / b
		[[nodiscard]] bool is_greater(const int64_t a, const int64_t b) const
		{
	...
	public:
		...
		void resize(const std::size_t newSize) { size_ = newSize; }
