...
namespace Stockfish {
	...
	TTEntry* TranspositionTable::probe(const Key key, bool& found) const {
		...
		for (int i = 0; i < ClusterSize; ++i)
			...
			{
				tte[i].genBound8 = static_cast<uint8_t>(generation8 | (tte[i].genBound8 & (GENERATION_DELTA - 1))); // Refresh

				return found = static_cast<bool>(tte[i].depth8), &tte[i];
			}
	...
	int TranspositionTable::hashfull() const {
		...
		for (int i = 0; i < 1000; ++i)
			for (auto& j : table[i].entry)
				cnt += j.depth8 && (j.genBound8 & GENERATION_MASK) == generation8;
