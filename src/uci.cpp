...
namespace Stockfish {
	...
	namespace {
		...
		int win_rate_model(const Value v, const int ply) {
			...
			// logistic function.
			constexpr double as[] = { -3.68389304,  30.07065921, -60.52878723, 149.53378557 };
			constexpr double bs[] = { -2.0181857,   15.85685038, -29.83452023,  47.59078827 };
			const double a = (((as[0] * m + as[1]) * m + as[2]) * m) + as[3];
			const double b = (((bs[0] * m + bs[1]) * m + bs[2]) * m) + bs[3];

			// Transform eval to centipawns with limited range
			const double x = std::clamp(static_cast<double>(100 * v) / static_cast<double>(PawnValueEg), -2000.0, 2000.0);

			// Return win rate in per mille (rounded to nearest)
			return lround((0.5 + 1000.0 / (1 + std::exp((a - x) / b))));
		}
