...
namespace Stockfish {
	...
	namespace Eval {
		...
		void NNUE::init() {
			...
			useNNUE = static_cast<bool>(Options["Use NNUE"]);
			if (!useNNUE)
				return;

			auto eval_file = string(Options["EvalFile"]);
			if (eval_file.empty())
...
#define stringify(x) stringify2(x)
...
#else
			const vector<string> dirs = { "<internal>" , "" , CommandLine::binaryDirectory };
#endif
			...
			for (const string& directory : dirs)
				if (currentEvalFileName != eval_file)
				{
					...
					{
						if (ifstream stream(directory + eval_file, ios::binary); load_eval(eval_file, stream))
					...
					if (directory == "<internal>" && eval_file == EvalFileDefaultName)
					{
						...
						public: MemoryBuffer(char* p, const size_t n) { setg(p, p, p + n); setp(p, p + n); }
						};

						MemoryBuffer buffer(const_cast<char*>(reinterpret_cast<const char*>(gEmbeddedNNUEData)),
							(gEmbeddedNNUESize));

						if (istream stream(&buffer); load_eval(eval_file, stream))
							currentEvalFileName = eval_file;
					}
				}
		}
		...
		void NNUE::verify() {
			...
			{
				const string msg1 = "If the UCI option \"Use NNUE\" is set to true, network evaluation parameters compatible with the engine must be available.";
				const string msg2 = "The option is set to true, but the network file " + eval_file + " was not loaded successfully.";
				const string msg3 = "The UCI option EvalFile might need to specify the full path, including the directory name, to the network file.";
				const string msg4 = "The default net can be downloaded from: https://tests.stockfishchess.org/api/nn/" + std::string(EvalFileDefaultName);
				const string msg5 = "The engine will be terminated now.";
		...
		double to_cp(const Value v) { return static_cast<double>(v) / static_cast<double>(PawnValueEg); }
	...
	namespace {
		...
		constexpr Value LazyThreshold1 = static_cast<Value>(3631);
		constexpr Value LazyThreshold2 = static_cast<Value>(2084);
		constexpr Value SpaceThreshold = static_cast<Value>(11551);
...
#define S(mg, eg) make_score(mg, eg)
		...
		constexpr Value CorneredBishop = static_cast<Value>(50);
...
#undef S
		...
		void Evaluation::initialize() {
			...
			const Square s = make_square(std::clamp(file_of(ksq), FILE_B, FILE_G),
		...
		Score Evaluation::pieces() {
			...
			while (b1)
			{
				...
				if (Pt == BISHOP || Pt == KNIGHT)
				{
					...
					Bitboard bb = OutpostRanks & (attackedBy[Us][PAWN] | shift<Down>(pos.pieces(PAWN)))
						& ~pe->pawn_attacks_span(Them);

					if (Bitboard targets = pos.pieces(Them) & ~pos.pieces(PAWN); Pt == KNIGHT
		...
		template<Color Us>
		[[nodiscard]] [[nodiscard]] Score Evaluation::king() const {
			...
			kingDanger += kingAttackersCount[Them] * kingAttackersWeight[Them] // (~10 Elo)
				...
				- 100 * static_cast<bool>(attackedBy[Us][KNIGHT] & attackedBy[Us][KING])  // (~5 Elo)
   ...
   // Transform the kingDanger units into a Score, and subtract it from the evaluation
		...
		template<Color Us>
		[[nodiscard]] [[nodiscard]] Score Evaluation::threats() const {
			...
			{
				const bool queenImbalance = pos.count<QUEEN>() == 1;
		...
		template<Color Us>
		[[nodiscard]] [[nodiscard]] Score Evaluation::passed() const {
		...
		template<Color Us>
		[[nodiscard]] [[nodiscard]] Score Evaluation::space() const {
		...
		Value Evaluation::winnable(const Score score) const {
			...
			const int u = ((mg > 0) - (mg < 0)) * std::clamp(complexity + 50, -abs(mg), 0);
		...
		Value Evaluation::value() {
			...
			auto lazy_skip = [&](const Value lazyThreshold) {
		...
		Value fix_FRC(const Position& pos) {
			if (constexpr Bitboard Corners = 1ULL << SQ_A1 | 1ULL << SQ_H1 | 1ULL << SQ_A8 | 1ULL << SQ_H8; !(pos.pieces(BISHOP) & Corners))
				return VALUE_ZERO;
			...
			return pos.side_to_move() == WHITE ? static_cast<Value>(3 * correction)
				: -static_cast<Value>(3 * correction);
		}
	...
	Value Eval::evaluate(const Position& pos) {
		...
		{
			const Value nnue = NNUE::evaluate(pos, true);     // NNUE
			const int scale = 1136 + 20 * pos.non_pawn_material() / 1024;
			const Color stm = pos.side_to_move();
			Value optimism = pos.this_thread()->optimism[stm];
			const Value psq = (stm == WHITE ? 1 : -1) * eg_value(pos.psq_score());
			const int complexity = 35 * abs(nnue - psq) / 256;
