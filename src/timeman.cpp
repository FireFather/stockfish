...
namespace Stockfish {
	...
	void TimeManagement::init(Search::LimitsType& limits, const Color us, const int ply) {
		...
		const double optExtra =
			std::clamp(1.0 + 12.0 * static_cast<double>(limits.inc[us]) / static_cast<double>(limits.time[us]), 1.0, 1.12);
		...
		maximumTime = static_cast<TimePoint>(std::min(0.8 * static_cast<double>(limits.time[us])
			- static_cast<double>(moveOverhead), maxScale * static_cast<double>(optimumTime)));

		if (static_cast<bool>(Options["Ponder"]))
