...
#define vec_sub_psqt_32(a,b) _mm256_sub_epi32(a,b)
#define vec_zero_psqt() _mm256_setzero_si256()
constexpr auto NumRegistersSIMD = 16;
...
#pragma GCC diagnostic pop
		...
		std::int32_t transform(const Position& pos, OutputType* output, const int bucket) const {
			...
			for (IndexType p = 0; p < 2; ++p)
			{
				...
				for (IndexType j = 0; j < NumChunks; ++j)
				{
					...
					const __m256i sum0 = _mm256_load_si256(&reinterpret_cast<const __m256i*>
						(accumulation[perspectives[p]])[j * 2 + 0]);
					const __m256i sum1 = _mm256_load_si256(&reinterpret_cast<const __m256i*>
						(accumulation[perspectives[p]])[j * 2 + 1]);
			...
			for (IndexType p = 0; p < 2; ++p)
			{
				...
				{
					const BiasType sum = accumulation[perspectives[p]][j];
	...
	// is defined in the VECTOR code below, once in each branch
			...
			else
			{
				...
				for (IndexType j = 0; j < HalfDimensions / TileHeight; ++j)
				{
					const auto biasesTile = reinterpret_cast<const vec_t*>(
						&biases[j * TileHeight]);
					for (IndexType k = 0; k < NumRegs; ++k)
						acc[k] = biasesTile[k];
					...
					const auto accTile = reinterpret_cast<vec_t*>(
				...
				for (IndexType j = 0; j < PSQTBuckets / PsqtTileHeight; ++j)
				{
					...
					const auto accTilePsqt = reinterpret_cast<psqt_vec_t*>(
