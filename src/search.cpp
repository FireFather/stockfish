...
namespace Stockfish {
	...
	namespace {
		...
		Value futility_margin(const Depth d, const bool improving) {
			return static_cast<Value>(214 * (d - improving));
		}
		...
		Depth reduction(const bool i, const Depth d, const int mn, const Value delta, const Value rootDelta) {
			...
			return (r + 1358 - static_cast<int>(delta) * 1024 / static_cast<int>(rootDelta)) / 1024 + (!i && r > 904);
		}
		...
		ExplosionState search_explosion(Thread* thisThread) {
			...
			const bool explosive = thisThread->doubleExtensionAverage[WHITE].is_greater(2, 100)
		...
		struct Skill {
			Skill(const int skill_level, const int uci_elo) {
				if (uci_elo)
					level = std::clamp(std::pow((uci_elo - 1346.6) / 143.4, 1 / 0.806), 0.0, 20.0);
				else
					level = static_cast<double>(skill_level);
			}

			[[nodiscard]] bool enabled() const { return level < 20.0; }
			[[nodiscard]] bool time_to_pick(const Depth depth) const { return depth == 1 + static_cast<int>(level); }
	...
	void Search::init() {
		...
		for (int i = 1; i < MAX_MOVES; ++i)
			Reductions[i] = static_cast<int>((21.9 + std::log(Threads.size()) / 2) * std::log(i));
	}
	...
	void MainThread::search() {
	   ...
	   // "ponderhit" just reset Threads.ponder).
		...
		if (const auto skill = Skill(Options["Skill Level"], Options["UCI_LimitStrength"] ? static_cast<int>(Options["UCI_Elo"]) : 0); static_cast<int>(Options["MultiPV"]) == 1
			&& !Limits.depth
			&& !skill.enabled()
	...
	void Thread::search() {
		...
		trend = SCORE_ZERO;
		optimism[us] = static_cast<Value>(25);
			...
			&& !(Limits.depth && mainThread && rootDepth > Limits.depth))
		{
			...
			for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
			{
				...
				if (rootDepth >= 4)
				{
					...
					delta = static_cast<Value>(17) + static_cast<int>(prev) * prev / 16384;
					alpha = std::max(prev - delta, -VALUE_INFINITE);
					beta = std::min(prev + delta, VALUE_INFINITE);

					// Adjust trend and optimism based on root move's previousScore
					const int tr = sigmoid(prev, 0, 0, 147, 113, 1);
				...
				&& !mainThread->stopOnPonderhit)
			{
				...
				const double bestMoveInstability = 1.073 + std::max(1.0, 2.25 - 9.9 / rootDepth)
	...
	namespace {
		...
		Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
				...
				&& (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
			{
				...
				Depth R = std::min(static_cast<int>(eval - beta) / 205, 3) + depth / 3 + 4;
			...
			while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
			{
					...
					&& bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
				{
						...
						|| givesCheck)
					{
						...
						if (!pos.see_ge(move, static_cast<Value>(-217) * depth))
							continue;
					}
					else
					{
						...
						if (!pos.see_ge(move, static_cast<Value>(-21 * lmrDepth * lmrDepth - 21 * lmrDepth)))
							continue;
					}
				}
		...
		Value qsearch(Position& pos, Stack* ss, Value alpha, const Value beta, const Depth depth) {
			...
			const Value ttValue = ss->ttHit ? value_from_tt(tte->value(), ss->ply, pos.rule50_count()) : VALUE_NONE;
			const Move ttMove = ss->ttHit ? tte->move() : MOVE_NONE;
			const bool pvHit = ss->ttHit && tte->is_pv();
		...
		void update_quiet_stats(const Position& pos, Stack* ss, const Move move, const int bonus) {
		...
		Move Skill::pick_best(const size_t multiPV) {
			...
			const double weakness = 120 - 2 * level;
			...
			for (size_t i = 0; i < multiPV; ++i)
			{
				...
				if (const int push = static_cast<int>((weakness * static_cast<int>(topScore - rootMoves[i].score)
					+ delta * (rng.rand<unsigned>() % static_cast<int>(weakness))) / 128); rootMoves[i].score + push >= maxScore)
				{
