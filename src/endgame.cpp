...
namespace Stockfish {
	...
	Value Endgame<KRKP>::operator()(const Position& pos) const {
		...
		if ((forward_file_bb(strongSide, strongKing) & weakPawn)
			|| ((distance(weakKing, weakPawn) >= 3 + (pos.side_to_move() == weakSide))
	...
	ScaleFactor Endgame<KRPPKRP>::operator()(const Position& pos) const {
		...
		const Square strongPawn1 = lsb(pos.pieces(strongSide, PAWN));
		const Square strongPawn2 = msb(pos.pieces(strongSide, PAWN));
		const Square weakKing = pos.square<KING>(weakSide);
	...
	ScaleFactor Endgame<KPsK>::operator()(const Position& pos) const {
		...
		if (Bitboard strongPawns = pos.pieces(strongSide, PAWN); !(strongPawns & ~(FileABB | FileHBB))
